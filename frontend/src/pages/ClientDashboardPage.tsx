    ...
    <main className="mx-auto max-w-7xl px-4 py-10 text-white sm:px-6 lg:px-8">
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-[1.05fr_0.95fr]">
        <section className="rounded-[32px] border border-white/10 bg-white/5 p-6 shadow-panel backdrop-blur">
          ...
          <h2 className="mt-3 text-3xl font-display text-white">Все, что нужно для сопровождения проекта</h2>

          <div className="mt-6 grid gap-4">
            {workspaceModules.map((module) => (
              <article key={module.title} className="rounded-[28px] border border-white/10 bg-[#102435] p-5">
                <h3 className="text-xl font-display text-white">{module.title}</h3>
                <p className="mt-3 text-sm leading-7 text-mist/75">{module.description}</p>
        ...
        <div className="space-y-6">
          <section className="rounded-[32px] border border-white/10 bg-white/5 p-6 shadow-panel backdrop-blur">
            ...
            <h2 className="mt-3 text-2xl font-display text-white">Прозрачность работы без лишней переписки</h2>
            <div className="mt-5 space-y-3">
              {clientBenefits.map((item) => (
                <div key={item} className="rounded-2xl border border-white/10 bg-[#102435] px-4 py-4 text-sm leading-7 text-white/85">
                  {item}
                </div>
              ))}
            </div>
          ...
          <section className="rounded-[32px] border border-white/10 bg-white/5 p-6 shadow-panel backdrop-blur">
            ...
            <h2 className="mt-3 text-2xl font-display text-white">Материалы проекта доступны только авторизованным пользователям</h2>
            <div className="mt-5 grid gap-3">
              <div className="rounded-2xl border border-white/10 bg-[#102435] px-4 py-4 text-sm text-mist/75">Защищенный вход в рабочую зону</div>
              <div className="rounded-2xl border border-white/10 bg-[#102435] px-4 py-4 text-sm text-mist/75">Разграничение доступа по ролям</div>
              <div className="rounded-2xl border border-white/10 bg-[#102435] px-4 py-4 text-sm text-mist/75">Единая история взаимодействия по проекту</div>
            </div>
          </section>
        </div>
      </div>
